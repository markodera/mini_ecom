...
class SMSService:
    ...
    def _redis_key(self, phone_number, suffix):
        ...
        phone_clean = (
            str(phone_number).replace("+", "").replace(" ", "").replace("-", "")
        )
    ...
    def _can_send_code_db(self, phone_number):
        ...
        recent_codes = PhoneVerification.objects.filter(
            phone_number=phone_number, created_at__gte=one_hour_ago
        ).count()
    ...
    def store_verification_code(self, phone_number, code, user_id):
        ...
        logger.info(
            f"Verification stored: user={user_id} phone={str(phone_number)[:8]}***"
        )
        return verification
    ...
    def _verify_code_redis(self, phone_number, submitted_code, user_id):
        ...
        max_attempts_per_min = settings.PHONE_VERIFICATION[
            "RATE_LIMIT_VERIFICATIONS_PER_MINUTE"
        ]
        ...
        try:
            ...
            if (
                stored_data["code"] == submitted_code
                and stored_data["user_id"] == user_id
            ):
    ...
    def _verify_code_db(self, phone_number, submitted_code, user_id):
        ...
        verification = (
            PhoneVerification.objects.filter(
                phone_number=phone_number, user_id=user_id, verified_at__isnull=True
            )
            .order_by("-created_at")
            .first()
        )
...
def send_phone_verification(user, phone_number, sms_service: SMSService):
    ...
    if success:
        ...
        return {"success": False, "error": "sms_failed", "detail": result}
