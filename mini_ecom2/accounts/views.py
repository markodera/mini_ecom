...
class SocialLogin2FAMixin:
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.session.get("requires_2fa") and not request.session.get(
            "otp_verified"
        ):
    ...
    def complete_login(self, request, user, is_backup=False):
        ...
        if is_backup:
            response_data[
                "warning"
            ] = "Backup code used. Please generate new backup codes"
...
class CustomLoginView(DjRestAuthLoginView):
    ...
    def verify_2fa_and_login(self, request):
        ...
        if not user.is_active:
            return Response(
                {"detail": "User account disabled"}, status=status.HTTP_403_FORBIDDEN
            )
        # Totp first (6 digits)
        verified = False
        is_backup = False
        ...
        return Response(
            {"detail": "Invalid 2FA token."}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    def post(self, request):
        ...
        if not user.is_active:
            try:
                ...
                return Response(
                    {"detail": "Verification email sent.", "email": email},
                    status=status.HTTP_200_OK,
                )
...
def send_verification_code(request):
    ...
    if result["success"]:
        ...
        response_data = {"detail": result["detail"], "type": result["error"]}
...
def verify_phone_number(request):
    ...
    is_valid, message, is_backup = sms_service.verify_code(
        phone_number, code, request.user.id
    )
    ...
    if not is_valid:
        ...
        return Response(
            {"detail": message, "type": "invalid_code"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    return Response(
        {
            ...
            "code_expires_in_minutes": settings.PHONE_VERIFICATION[
                "CODE_EXPIRY_MINUTES"
            ],
        },
        status=status.HTTP_200_OK,
